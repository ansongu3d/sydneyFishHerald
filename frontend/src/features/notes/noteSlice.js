...
export const getNotes = createAsyncThunk(
  ...
  async (topicId, thunkAPI) => {
    try {
      ...
      return await noteService.getNotes(topicId, token)
    } catch (error) {
...
export const createNote = createAsyncThunk(
  'notes/create',
  async ({ noteText, topicId }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token
      return await noteService.createNote(noteText, topicId, token)
    } catch (error) {
      return thunkAPI.rejectWithValue(extractErrorMessage(error))
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getNotes.fulfilled, (state, action) => {
        // NOTE: even if there are no notes for the topic we get an empty
